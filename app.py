...
USERS = {"hima": "krishi123", "farmer": "pass"} 
...
def show_knowledge_base():
    ...
    st.markdown("## 📊 Offline Knowledge Base Contents")
...
def get_voice_input(lang_code):
    ...
    r = sr.Recognizer()
    st.info("🎙️ Listening... Please speak your crop issue clearly.")
...
FALLBACK_ADVICE_HI = "स्थानीय डेटाबेस में कोई मिलान नहीं मिला। कृपया अपने लक्षणों को और स्पष्ट करें।"
FALLBACK_ADVICE_TE = "స్థానిక డేటాబేస్లో సరిపోలేది కనుగొనబడలేదు. దయచేసి మీ లక్షణాలను మెరుగుపరచండి।"
...
# ----------------- PAGE CONFIG -----------------
st.set_page_config(
    page_title="Krishi Sajag — AI Crop Advisor",
    page_icon="🌾",
    layout="wide"
)
...
/* 🌟 NEW FIX: Force background of radio options to dark green for visibility */
...
# ----------------- HEADER -----------------
st.markdown('<p class="title">🌾 Krishi Sajag</p>',unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.session_state['authenticated']:
        ...
        if st.button("🧠 AI Crop Advisor", key="nav_advisor"):
            st.session_state.page = 'advisor'
        if st.button("📊 View Knowledge Base", key="nav_database"):
        ...
        st.markdown("### 🔑 Login to Krishi Sajag")
        ...
        st.info("Demo User: hima / krishi123")
        
    st.markdown("---")
    st.caption("Built by Hima • Powered by Gemini 🌿")
...
if not st.session_state['authenticated']:
    ...
    if st.session_state.page == 'database':
        ...
        with st.container(border=True): 
            st.markdown("## 1. 🌿 Describe Your Crop Issue")
            ...
            with col1:
                ...
                if st.button("🎤 Use Voice Input (Offline)", key="voice_button_main"):
        ...
        if st.button("✨ Start AI Diagnosis"):
            ...
            if not query and not uploaded_file:
                st.warning("⚠️ Please **describe** a crop issue or **upload** a photo in the sections above.")
            else:
                with st.spinner("Analyzing your crop issue using local knowledge... 🌿"):
                    
                    advice = get_offline_advice(query, current_lang_code)
                    
                    if advice:
                        st.success(f"✅ Diagnosis Found (Offline in {selected_language})!")
                        
                        # Dynamic header creation logic
                        if current_lang_code == 'hi':
                            symptom_header = "#### 🔍 Symptoms / लक्षण"
                            cause_header = "#### 🤔 Cause / कारण"
                            remedy_header = "#### 💊 Remedies (Low-Cost/Organic) / उपचार"
                            preventive_header = "#### 🛡️ Preventive Measures / रोकथाम"
                        elif current_lang_code == 'te':
                            symptom_header = "#### 🔍 Symptoms / లక్షణాలు"
                            cause_header = "#### 🤔 Cause / కారణం"
                            remedy_header = "#### 💊 Remedies (Low-Cost/Organic) / నివారణ"
                            preventive_header = "#### 🛡️ Preventive Measures / నివారణ చర్యలు"
                        else:
                            symptom_header = "#### 🔍 Symptoms"
                            cause_header = "#### 🤔 Cause"
                            remedy_header = "#### 💊 Remedies (Low-Cost/Organic)"
                            preventive_header = "#### 🛡️ Preventive Measures"

                        st.markdown(f"### 🌾 {advice['disease_name']} — Suggested Diagnosis & Remedies")
                        ...
                        st.warning("⚠️ Local match failed. Attempting to use live AI (requires internet/API).")
                        ...
                        if MODEL:
                            ...
                            try:
                                ...
                                **Translate the entire response into {selected_language}.**
                                ...
                                st.success("✅ AI Analysis Complete (Online Fallback)!")
                                st.markdown(f"### 🌾 Suggested Diagnosis & Remedies\n{response.text}")
                            except Exception as e:
                                st.error(f"❌ Error during online AI analysis: {fallback_text}")
                        else:
                            st.error(f"❌ Gemini Model is not configured or there is no internet connection. Cannot provide diagnosis. {fallback_text}")
